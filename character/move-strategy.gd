static func go(direction, acceleration, max_speed=0, motion=Vector2.ZERO, delta=0):
...
static func apply_friction(amount, motion):
...
static func apply_movement(acceleration, motion, max_speed):
	motion += acceleration
